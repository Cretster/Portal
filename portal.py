...
def build_dual_trend_chart(dates, day_max, night_min, rain, growth_array, presence_array, rh_avg, wind_max, species_name):
    ...
    fig.add_trace(go.Scatter(x=dates, y=growth_array, mode="lines", name="<b>⚡ ACTIVE NEW GROWTH %</b>", line=dict(color="#e74c3c", width=5, dash="dot"), yaxis="y2"))
    fig.add_trace(go.Scatter(x=dates, y=presence_array, mode="lines", name="<b>🍄 EXISTING GROWTH FIND %</b>", line=dict(color="#2ecc71", width=5), yaxis="y2"))
