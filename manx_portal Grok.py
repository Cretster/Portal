...
def ph_legend_html(ph_value=None):
    ...
    parts = ['<div style="font-size:13px;line-height:1.5">']
    parts.append("<b>Soil pH (0–5 cm) — SoilGrids prediction</b><br>")
    for colour, rng, desc in rows:
        parts.append(
            f'<span style="display:inline-block;width:18px;height:12px;'
            f'background:{colour};border:1px solid #999;margin-right:6px"></span>'
            f'{rng} &nbsp; <span style="color:#555">{desc}</span><br>'
        )
    if ph_value is not None:
        parts.append(
            f'<div style="margin-top:8px;font-size:16px">'
            f'Clicked location pH ≈ <b style="color:#1a5276">{ph_value:.1f}</b></div>'
        )
    else:
        parts.append('<div style="margin-top:8px;color:#666">Click the map to read pH at that point.</div>')
    parts.append(
...
def build_clickable_ph_map(center_lat=54.23, center_lon=-4.55, zoom=10, clicked=None, grid_meta=None):
    """
    Folium map with custom-coloured SoilGrids pH image overlay (matches the
    standalone HTML colour scheme). Falls back to ISRIC WMS if the PNG is missing.
    """
    m = folium.Map(
    ...
    if png and grid_meta:
        ImageOverlay(
            name="Soil pH 0–5 cm",
            image=png,
            bounds=[[grid_meta["south"], grid_meta["west"]],
        ...
        # Fallback: live WMS (different default colour ramp)
    ...
    if clicked and clicked.get("lat") is not None:
        folium.Marker(
            ...
            popup=f"{clicked['lat']:.5f}, {clicked['lon']:.5f}",
...
if app_mode == "📍 Hyperlocal Focused Zone":
    ...
    if location_mode == "📮 Postcode Zone":
        ...
        st.sidebar.info("Click anywhere on the map to set the location. Weather, scores and pH update automatically.")
        ...
        fmap = build_clickable_ph_map(center_lat, center_lon, zoom, st.session_state.map_click, ph_grid)
        ...
        if map_data and map_data.get("last_clicked"):
            ...
            st.session_state.map_click = {"lat": lat, "lon": lon}
        ...
        # Legend + pH readout directly under the map
        st.markdown(ph_legend_html(current_ph), unsafe_allow_html=True)
        if ph_grid is None:
            st.warning("Soil pH grid file (`iom_ph_grid.json`) not found next to the app — pH values cannot be sampled. Overlay may fall back to WMS.")
    ...
    if location_mode != "🗺️ Click on soil-pH map":
        ...
        if current_ph is not None:
            st.caption(f"**Predicted soil pH (0–5 cm):** {current_ph:.1f}")
        st.markdown("---")
