...
st.caption("1) Use the Zoom (+/-) buttons (for reliability compare to screen pinch), then Click on a location you're interested in on the map.")
...
st.caption("4) This indicates the chance of growth for that location as long as the pH value of the soil is ideal")
st.caption("v) You can see also historical weather data (and adjust the number of days to be shown) so that the graph will show you all the data in a linear fashion")
st.caption("Six) So that you can then observe from the thicker multicolour line on the graph any points in the last X days where growth would have been likely")
st.caption("8: in that location and may still be found etc.")
st.caption("7- Finally there is a second map which shows random samples of places which currently have a moderate to high chance of growth at the moment.")
